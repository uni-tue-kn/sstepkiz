...
$DefaultSensorConfigPath = "$SSTEPKIZ_DIR\backends\sensor-configuration-example.json"
# The path where the sensor-configuration.json should be placed
$DestinationFilePath = "$SSTEPKIZ_DIR\backends\sensor-configuration.json"
...
if ($DefaultUse) {
  ...
  if (-Not ($currentPath -Split ";" -Contains "$SSTEPKIZ_DIR\tools\apdm-server\lib\Windows\x64")) {
    $newPath = $currentPath + ";$SSTEPKIZ_DIR\tools\apdm-server\lib\Windows\x64"
